...
const LogIn = () => {
  ...
  const submitUserData =async(e)=>{
   ...
   if( !email || !password){
     return alert("All feilds are required!")
      ...
      const response = await axios.post('https://achieverbackend.onrender.com/login',{
  ...
  return (
    <>
 { !changePasswordComponent ? <div className='flex justify-between'>
...
<p  className='text-xl text-white leading-6 font-montserrat leading-8 mt-2 ml-6 font-bold'>Unlock a world of<br/> knowledge and possibilities!</p>
...
<div className='absolute z-10  ml-[430px]  rounded-l-xl h-[608px] bg-[#000000] w-[650px] text-white text-center'>
 <div className='ml-32 mt-24'>
    <h1 className='font-montserrat font-bold leading-9 text-3xl'>Welcome Back!</h1>
...
<input type='email' name='email' onChange={(e)=>setuserinput({...userinput,email:e.target.value})} placeholder='Enter Email Address' style={{background:"none"}} className='rounded-full w-[500px] ml-32 mt-3 border-2 text-sm h-9 p-3 border-[#8B8989] '/>

<input type='password' name='password'  onChange={(e)=>setuserinput({...userinput,password:e.target.value})} placeholder='Enter Password' className='rounded-full w-[500px] ml-32 mt-3 border-2 h-9 text-sm p-3 border-[#8B8989] bg-[#100D0F]'/>
...
<p>Remember Me</p>
<p onClick={()=>navigate("/setNewPassword")} className='ml-64 cursor pointer'>Forget Password?</p>
</span>
</div>
...
<p className='font-lato  font-normal ml-12 text-[#8E8B8B]'>Don't have an account?</p>
<Link to="/signup"><button className=' text-[#FF6584] text-sm'>Click Here</button></Link>
</span>
</form>
</div>
</div>

:<ForgotPassword setChangePasswordComponent={setChangePasswordComponent}/>}
    </>
  )
}
